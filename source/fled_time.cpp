...
TIMED_IS_READY::TIMED_IS_READY()
{
    TIME_START = std::chrono::steady_clock::now();
}

double TIMED_IS_READY::tir_now()
{
    auto tmeNow = std::chrono::steady_clock::now();
    std::chrono::duration<double> dur = tmeNow - TIME_START;
    double nowtime = dur.count();
...
void TIMED_IS_READY::set_e(double delay)
// Prep the TIMED_IS_READY varable
// delay is measured in ms.
{
...
bool TIMED_IS_READY::is_ready_e()
{
    // Check to see if enough time has passed.
    //  Returns true if interval time has passed.
    //    Resets timer if returned true.
...
double EFFICIANTCY_TIMER_EASY::eff_now()
{
    auto tmeNow = std::chrono::steady_clock::now();
    std::chrono::duration<double> dur = tmeNow - TIME_START;
    double nowtime = dur.count();
...
double EFFICIANTCY_TIMER_EASY::start_to_start_time()
{
    return START_TO_START_TIME;
}

void EFFICIANTCY_TIMER_EASY::start_timer()
{
    START_TO_START_TIME = eff_now() - TIMER_STARTED;
    TIMER_STARTED = eff_now();
}

double EFFICIANTCY_TIMER_EASY::elapsed_timer_time()
{
    return eff_now() - TIMER_STARTED;
}

double EFFICIANTCY_TIMER_EASY::elapsed_time()
{
    double time_elapsed = eff_now() - LAST_ASKED_TIME;
    LAST_ASKED_TIME = eff_now();
    return time_elapsed;
}

double EFFICIANTCY_TIMER_EASY::simple_elapsed_time()
{
  return eff_now() - TIMER_STARTED;
}
