...
namespace {
    ...
    // win_tools' bottom-row footer (OLLI_GIT_VERSION comes from version.h,
    // generated by CMake - see GenerateVersionHeader.cmake). "v." prefix per
    // request; a single named constant here rather than recomputing it in
    // both ncurses_layout() (sizing tools_w to fit it) and
    // display_with_ncurses() (passing it to ncurses_render_panel()) keeps
    // the two from ever disagreeing on what string they're each reasoning
    // about.
    const std::string TOOLS_PANEL_VERSION_FOOTER = std::string("v.") + OLLI_GIT_VERSION;
...
void OUTPUT_CLASS::ncurses_layout()
{
    ...
    // Widened past its 22-column base, if needed, so
    // TOOLS_PANEL_VERSION_FOOTER never gets truncated - +2 matches
    // ncurses_render_panel()'s own content_col=1 margin on each side for
    // the tools panel.
    const int tools_w = std::max(22, static_cast<int>(TOOLS_PANEL_VERSION_FOOTER.size()) + 2);
    const int gap = 1;
...
// footer, if non-empty, claims win's own last row for itself - taken out of
// viewport_h below so scrollable content never overlaps it.
                                         ...
                                         const std::string& footer)
{
    ...
    int viewport_h = h - start_row - (footer.empty() ? 0 : 1);
    ...
    if (!footer.empty()) mvwaddstr(win, h - 1, content_col, footer.c_str());
