...
void OLLI_LINK::service(bool socket_readable)
{
    ...
    // running this tool for real - see the profile's history.json for
    // what this looked like from olli's side. Suppressing the ping for
    // this one tick defers it at most until the next 200ms tick, which is
    // nothing next to PING_INTERVAL_SECONDS.
